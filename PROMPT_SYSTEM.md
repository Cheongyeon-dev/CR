...
- [단서 발견 연출]: 특정 행동으로 새로운 단서를 최초로 발견했을 때, `<CustomComponent type="item_discover" id="log_01" />` 형태로 출력. (화면에 해당 단서의 텍스트와 디자인이 크게 렌더링 됨. id는 진행에 맞춰 log_01, log_02, log_03 등 점진적으로 제공)
